...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        /*
        * java.io패키지의 입출력스트림은 기본(기반)스트림과 필터(보조)스트림으로 분류할수있다
        * 기본 스트림은 외부데이터에 직접연결되는 스트림이고
        * 필터스트림은 외부데이터에 직접연결하는것이 아니라 기본스트림에 추가로 사용가능한 스트림이다
        * 주로 성능을 향상시키는 목적으로 사용된다
        * 생성자쪽에 매개변수로 다른 스트림을 이용하는 클래스를 필터스트림이라고 할수있다
        *
        * */
        ...
        try {
            ...
            //버퍼를 이용하는경우 버퍼가 가득차지않는 상태에서 내보낼땐
            //flush로 강제로 내보내기를 할수있다
            bw.flush();
            ...
            if(bw != null){
                try {
                    ...
                    //close를 호출하면 내부적으로 flush를 하고서 자원을 반납한다
                    bw.close();
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
